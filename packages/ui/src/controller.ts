...
export function createSpectrumUi(options: SpectrumUiOptions): SpectrumUi {
  ...
  function writePredictionChunk(
    ...
    const previousChunkCount = analysisSession.predictionChunkCount;
    const startIndex = Math.max(
      0,
      Math.min(previousChunkCount, nextChunkCount),
    );
    const nextArgmax = predictionArgmax.slice(startIndex, nextChunkCount);
    const nextConfidence = predictionConfidence
      ...
      : null;
    const writtenCount = Math.max(0, nextChunkCount - startIndex);
    if (writtenCount <= 0) {
    ...
    progressiveVisualizer.applyPredictionChunk({
      ...
      predictionOffset: analysisSession.predictionOffset,
    });
    analysisSession.predictionOffset += writtenCount;
    analysisSession.predictionChunkCount = nextChunkCount;
  }
  ...
  function resetAnalysisSession(token: number): void {
    ...
    analysisSession.predictionStepIndex = -1;
  ...
  function handleInferencePhase(event: AnalyzerInferencePhaseEvent): void {
    if (analysisSession.predictionStepIndex !== event.index) {
      analysisSession.predictionStepIndex = event.index;
      analysisSession.predictionChunkCount = 0;
    }
