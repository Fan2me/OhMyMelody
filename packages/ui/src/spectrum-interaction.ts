...
export function createSpectrumInteractionController(
  ...
  function bindMainCanvas(): void {
    ...
    let dragGesture: null | Record<string, never> = null;
    ...
    const handleDrag = (e: MouseEvent | PointerEvent | TouchEvent | null) => {
      const state = getState();
      if (state.spectrumW <= 0 || !dragGesture) return;
      markAutoPanSuppressed();
      ...
      const viewW = getMainViewFrameCount(state);
      const maxOffset = Math.max(0, state.spectrumW - viewW);
      const maxZoom = getDisplayMaxZoomForTotalFrames({
        totalFrames: state.spectrumW,
        maxUnitsPerSecond: getDisplaySamplingConfig().maxUnitsPerSecond,
      });
      const pageStepFrames = Math.max(
        1,
        getDisplayStrideFramesForZoom({
          zoom: state.spectrumZoom,
          minZoom: 1,
          maxZoom,
          minUnitsPerSecond: getDisplaySamplingConfig().minUnitsPerSecond,
          maxUnitsPerSecond: getDisplaySamplingConfig().maxUnitsPerSecond,
          frameRateHz: 100,
        }),
      );
      const edgeThreshold = Math.max(1, Math.floor(plotW * 0.1));
      let nextOffset = state.spectrumOffset;
      if (relX <= edgeThreshold) {
        nextOffset = Math.max(0, state.spectrumOffset - pageStepFrames);
      } else if (relX >= plotW - edgeThreshold) {
        nextOffset = Math.min(maxOffset, state.spectrumOffset + pageStepFrames);
      }
      if (nextOffset !== state.spectrumOffset) {
        setState({ spectrumOffset: nextOffset });
        requestSpectrumRedraw({
          dirtyMask: DIRTY.MAIN_BASE | DIRTY.MAIN_OVERLAY | DIRTY.OVERVIEW_OVERLAY,
        });
      }
      const frameFloat = nextOffset + (relX / plotW) * viewW;
    ...
    const startInteraction = (e: MouseEvent | PointerEvent | TouchEvent | any) => {
      ...
      const dragPoint = getClientPointFromEvent(e);
      if (dragPoint) {
        dragGesture = {};
      }
      markAutoPanSuppressed();
