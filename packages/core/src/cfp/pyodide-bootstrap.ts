...
export async function initializePyodideForCFP({
  ...
  if (!cfpScriptUrl) {
    throw new Error("cfp.py script url is required");
  }
  const response = await fetch(cfpScriptUrl);
  if (!response.ok) {
    throw new Error(`Failed to fetch cfp.py (HTTP ${response.status}). Ensure cfp.py is served from the core package path.`);
  }
  const code = await response.text();
  const htmlText = String(code || "").trim().slice(0, 200).toLowerCase();
  if (
    htmlText.startsWith("<!doctype") ||
    htmlText.startsWith("<html") ||
