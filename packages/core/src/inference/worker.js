...
function createResultFromSegments(id, inputName, batches) {
    ...
    for (const batch of batches) {
      ...
      for (const segment of segments) {
        ...
        self.postMessage({
          ...
          id,
          result: {
            totalArgmax: totalArgmax.slice(),
            totalConfidence: totalConfidence.slice(),
          },
        });
      }
    }
...
async function createPrioritySession(modelUrl) {
  ...
  await loadOrtRuntime(wasmScriptUrl);
  return createSessionWithProvider(modelUrl, "default");
}
  ...
  try {
    if (m.cmd === "init") {
      ...
      if (batches.length) {
        ...
        const result = await createResultFromSegments(id, inputName, batches);
      ...
      self.postMessage({ cmd: "progress", id, result });
      self.postMessage({ cmd: "result", id, result });
    }
  } catch (err) {
    self.postMessage({
