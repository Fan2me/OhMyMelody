...
const runtimePlanExports = await import("../dist/analysis-plan.js");
...
test("sample queue preserves order across partial drains", () => {
  const {
    createSampleQueue,
    enqueueSamples,
    takeSamples,
  ...
  enqueueSamples(queue, Float32Array.from([1, 2, 3]));
  enqueueSamples(queue, Float32Array.from([4, 5]));

  const first = takeSamples(queue, 4);
  const remaining = drainSamples(queue);
  const padded = padSamplesToLength(remaining, 4);

  assert.deepEqual(Array.from(first ?? []), [1, 2, 3, 4]);
  assert.deepEqual(Array.from(remaining), [5]);
  assert.deepEqual(Array.from(padded), [5, 0, 0, 0]);
});
...
test("analyzer emits phase events across multiple plan steps", async () => {
  ...
  const analyzer = createAnalyzer({
    ...
    inferenceManager: {
      reset() {},
      async process({ batches, complete, onProgress }) {
...
test("analyzer reuses completed CFP batches for the same file key", async () => {
  ...
  const analyzer = createAnalyzer({
    ...
    inferenceManager: {
      reset() {},
      async process({ batches, onProgress }) {
...
test("analyzer emits one inference result for complete CFP batches", async () => {
  const { AnalysisPhase, createAnalyzer } = runtimeExports;
...
test("InferenceManager merges incremental results and resets on model change", async () => {
  ...
  try {
    ...
    assert.deepEqual(initModels, ["mamba.onnx"]);
  } finally {
    globalThis.Worker = previousWorker;
  }
});

test("InferenceManager emits progress updates before final result", async () => {
  const { InferenceManager } = runtimeExports;
  const previousWorker = globalThis.Worker;
  ...
  try {
    ...
    const result = await manager.process({
      ...
      forceRefresh: true,
      onProgress(progress) {
        progressSnapshots.push(progress.totalArgmax.slice());
      },
    });

    assert.deepEqual(progressSnapshots, [[1], [1, 2]]);
    assert.deepEqual(result.totalArgmax, [1, 2]);
  } finally {
    globalThis.Worker = previousWorker;
  }
});
